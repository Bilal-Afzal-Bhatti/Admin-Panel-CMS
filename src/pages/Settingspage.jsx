...
import {
  ...
  Switch, FormControlLabel, MenuItem, Select, FormControl,
  InputLabel, CircularProgress, Alert, Snackbar, Chip, Tab, Tabs,
    ...
    <Section title="Admin Profile" subtitle="Update your personal information">
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2.5, mb: 3 }}>
        ...
        <Box>
          ...
          <Chip label={profile?.role} size="small" color="primary" sx={{ mt: 0.5, textTransform: 'capitalize' }} />
        </Box>
      </Box>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
        ...
        />
        <TextField
          label="Avatar URL (optional)" fullWidth
...
function PasswordTab({ showToast }) {
  ...
  return (
    <Section title="Change Password" subtitle="Use a strong password of at least 8 characters">
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
        <TextField
          label="Current Password" fullWidth
...
function StoreTab({ showToast }) {
  ...
  const [form, setForm] = useState({
    storeName: '', storeEmail: '', currency: 'USD', storeLogo: '', lowStockAlert: 10,
  });
  ...
  return (
    <Section title="Store Settings" subtitle="Manage your store information and preferences">
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
        ...
        <TextField
          label="Store Logo URL (optional)" fullWidth
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          ...
          <TextField
            label="Low Stock Alert (qty)" fullWidth type="number"
...
function NotificationsTab({ showToast }) {
  ...
  const toggle = (field) => {
    mutation.mutate({ ...notifs, [field]: !notifs[field] });
  };
...
function DangerZoneTab({ showToast }) {
  ...
  const handleDelete = () => {
    // Wire this to your delete admin API when ready
    ...
    <Paper sx={{
      ...
      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          ...
          <TextField
            fullWidth size="small" placeholder={CONFIRM_WORD}
            value={input} onChange={(e) => setInput(e.target.value)}
          />
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => { setConfirmOpen(false); setInput(''); }} color="inherit">
            Cancel
          </Button>
          <Button
    ...
    <Box>
      ...
      <Paper sx={{ borderRadius: 3, mb: 3, boxShadow: '0 1px 3px 0 rgb(0 0 0 / 0.08)' }}>
        ...
        >
          {TABS.map(({ label, icon }) => (
            <Tab
              key={label} label={label} icon={icon} iconPosition="start"
      ...
      {tab === 4 && <DangerZoneTab    showToast={showToast} />}

      {/* Global Toast */}
      <Snackbar
        open={toast.open} autoHideDuration={3000} onClose={hideToast}
